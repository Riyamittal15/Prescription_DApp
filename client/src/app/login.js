...
class NewLogin extends Component {
      ...
      if (existence === true) {
        ...
        if (role === "Insured") {
          ...
          this.setState({
            ...
            unknownUser: true,
            unverifiedUser: false,
            status: "registration_pharmacist",
          });
        }
      }
    } else {
  ...
  checkExistence = async (role, public_key) => {
    ...
    try {
      existenceInsured = await userContract.methods
        .checkExistence("insured", public_key)
    ...
    try {
      existencePhysician = await userContract.methods
        .checkExistence("physician", public_key)
        .call({ from: account, gas: 1000000 });
    } catch {
    ...
    try {
      existencePharmacist = await userContract.methods
        .checkExistence("pharmacist", public_key)
        .call({ from: account, gas: 1000000 });
    } catch {
    ...
    try {
      existenceVerifyingInstitution = await userContract.methods
        .checkExistence("verifying_institution", public_key)
        .call({ from: account, gas: 1000000 });
    } catch {
    ...
    if (
      existenceInsured === true ||
      existencePhysician === true ||
      existencePharmacist === true ||
      existenceVerifyingInstitution === true
    ) {
      existence = true;
    } else {
      existence = false;
  ...
  checkVerification = async (role, public_key) => {
    ...
    if (role === "Insured") {
      ...
      try {
        verified = await userContract.methods
          .checkVerification("verifying_institution", public_key)
          .call({ from: account, gas: 1000000 });
      } catch {
        verified = false;
      }
